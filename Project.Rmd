...
```{r Disaster By Year, fig.cap={"Diaster by year. Count of total disaster decreased from years $2005$ to $2008$. This is a decreasing trend because of we are focusing on a relatively short time span. "}}
...
ggplot(df_aggregate_1, aes(x = Year, y = total_disaster, color = ISO3)) +
  geom_line() +
  geom_point() +
  labs(title = "Count of Disaster by Year for Top Countries with Most Diasters",
       x = "Year", 
       y = "Sum of Disaster",
       color = "Country") 
```

In the scatterplot below, we explored the countries with most disaster by the log of CO2 emissions by metric tons. This graph only includes countries with greater than 10 events and is faceted by year. As you can see from this facet scatter plot, there is a lack of data for 2017 and 2018.
...
scatterplot_df %>% ggplot(aes(x = log(emission + 0.0001), 
  ...
  ggtitle("Emission versus Disaster Count for Each Country and Country
with Greater than 10 Diasters")
```
...
```{r CO2 Emissions by Type of Enterprise}
df_agg <- df_long %>% 
