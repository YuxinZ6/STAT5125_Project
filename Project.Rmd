...
By fitting the model with hyper-parameter combination suggested above, the model explains roughly $70%$ of the variation in the model. 
...
```{r K-Nearest Heldout Metrics}
metrics_knn_held
```
The models overall performed marginally better on the holdout data compared to the training data based on the MAE and RMSE. The best hyper-parameter in the test data is $k=10$ with MAE equal to 2.0 and RMSE with the value of 3.4. This model accounts for 48% of frequency.


# Model Selection
```{r Applying Bootstrap to the Best Model}
# set.seed(123457)
...
# Result Communication
The best model out of the five we have chosen is random forest.
