...
metrics_knn_held <- bind_rows(
  ...
  select(c(Model,.metric, .estimator, mean)) %>% 
  pivot_wider(names_from = c(.metric), 
                              values_from = mean)
```
...
metrics_knn_held
```
The models overall performed marginally better on the held-out data compared to the training data based on the MAE and RMSE. The best hyper-parameter in the test data is $k=10$ with MAE equal to 2.0 and RMSE with the value of 3.4. This model accounts for 48% of frequency.
