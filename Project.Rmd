...
# Visualizations
Climate change is a global issue as it leads to higher natural disasters. The line graph depicts the total disaster event by year on a global level.  
```{r Disaster By Year}
...
ggplot(df_aggregate_1, aes(x = Year, y = total_disaster, color = ISO3)) +
  geom_line() +
  geom_point() +
  labs(title = "Count of Disaster by Year for Top 10 Country with Most Diasters",
       x = "Year", 
       y = "Sum of Disaster",
       color = "Country") 
```
...
# Uncertainty Quantification
=======
>>>>>>> ae9fd43e9adf0e2e92312a4b7fd052d75769818f
