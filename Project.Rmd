...
df %>% 
  mutate_at(c("Sector_abb", "Year"), as.factor) %>%
  glimpse()
...
#Renaming sectors
df<- df %>% 
  rename(label = Sector_abb) %>% 
  left_join(Sectors_label, by = "label") %>%
  relocate(Sector, .after = Year) %>%
  select(-label)
...
df_long <- df %>% pivot_longer(cols = Export_Domestics:Disaster_Frequency,
                           names_to = "Category",
                           values_to = "Value")
df_wide <- df_long %>% pivot_wider(names_from = c("Sector", "Category"), 
                                   values_from = Value, 
                                   values_fill = 0,
                                   names_glue = "{Category}_{Sector}")
```
